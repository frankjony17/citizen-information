'use strict';
angular.module('sdsicApp').factory('comumService', ['$http', '$mdDialog', service]);
...
function service($http, $mdToast){
    ...
    var buscarUrlsMenusPorAmbiente = function(){
        return $http.get('/sdsic/api/cliente/menu');
    };
    ...
    var obterPermissoes = function(cpf, modulo){
        return $http.get('sdsic/comum/buscar-permissoes/' + cpf + '/' + modulo);
    };

    var buscarIdSistema = function(){
        return $http.get('sdsic/comum/buscar-id-sistema');
    };

    var logout = function(){
        return $http.get('sdsic/api/saml/logout');
    };
